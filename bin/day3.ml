...
let common_item (comp1, comp2) =
  let rec aux = function
    ...
    | [] -> None
  in
  aux (List.init (String.length comp1) (String.get comp1))

let priority = function
  | None -> 0
  | Some a ->
      1 + String.index "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ" a
...
let common_item3 (bp1, bp2, bp3) =
  let rec aux = function
    ...
    | [] -> None
  in
  aux (List.init (String.length bp1) (String.get bp1))

let () =
  let parsed = "files/day3.txt" |> Aoc_2k22.read_lines in
  parsed |> List.map line_to_backpack |> List.map common_item
  |> List.map priority |> Aoc_2k22.sum_list |> print_int;
  print_newline ();
  parsed |> group_input |> List.map common_item3 |> List.map priority
  |> Aoc_2k22.sum_list |> print_int
