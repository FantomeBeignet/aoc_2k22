...
let contains (sec1, sec2) = match sec1 with
...
let parse_section = function
  ...
  | _ -> raise (Invalid_argument "Invalid section format");;

let line_to_sections line =
  let split_line = String.split_on_char ',' line in
  match split_line with
  | a::b::[] -> (parse_section (String.split_on_char '-' a), parse_section (String.split_on_char '-' b));
  | _ -> raise (Invalid_argument "Invalid line format");;

let overlaps (sec1, sec2) = match sec1 with
...
let () =
  let parsed = "files/day4.txt" |> Aoc_2k22.read_lines |> List.map line_to_sections in 
